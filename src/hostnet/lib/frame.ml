...
type t =
  | Ethernet: { src: Macaddr.t; dst: Macaddr.t; payload: 'a } -> t
  | Arp:      { op: [ `Request | `Reply | `Unknown ] } -> t
  | Ipv4:     { src: Ipaddr.V4.t; dst: Ipaddr.V4.t; dnf: bool; payload: 'a } -> t
  | Udp:      { src: int; dst: int; len: int; payload: 'a } -> t
  | Tcp:      { src: int; dst: int; payload: 'a } -> t
    ...
    | Some src, Some dst ->
      ...
      ( match ethertype with
        | 0x0800 ->
          ...
          let inner = Cstruct.shift inner (4 * (vihl land 0xf)) in
          ( match proto with
            | 6 ->
